...
async function main() {
  ...
  const { data: u, error: uerr } = await admin.auth.admin.createUser({
    email, password: randomBytes(16).toString("hex"), email_confirm: true,
  });
  if (uerr) throw uerr;
  ...
  await admin.from("negotiations").insert({
    id: negotiationId, user_id: userId, title: `e2e ${runId}`,
    origin_address: "A", destination_address: "B",
    moving_date: future, bedroom_count: 2, vertical: "moving", workflow_status: "DRAFT",
  }).throwOnError();
  await admin.from("providers").insert({
    id: providerId, negotiation_id: negotiationId, name: "Acme Movers", phone: "+15550000000",
  }).throwOnError();
  await admin.from("job_specs").insert({
    negotiation_id: negotiationId, version: 1, specification: specDoc,
    specification_hash: specHash, confirmed: true,
  }).throwOnError();
  await admin.from("calls").insert({
    id: callId, negotiation_id: negotiationId, provider_id: providerId,
    agent_type: "elevenlabs", external_call_id: externalCallId,
    status: "in_progress", job_spec_version: 1, job_spec_hash: specHash,
  }).throwOnError();

  const rawToken = randomBytes(32).toString("hex");
  await admin.from("call_tool_tokens").insert({
    call_id: callId, token_hash: sha256Hex(rawToken),
    expires_at: new Date(Date.now() + 2 * 3600e3).toISOString(),
  }).throwOnError();

  const toolHeaders = { "X-BidPilot-Call-Token": rawToken };
  const specCtx = {
    call_id: callId, provider_id: providerId,
    expected_spec_version: 1, expected_spec_hash: specHash,
    conversation_id: externalCallId,
  };

  // ── 1. INITIAL quote ────────────────────────────────────────────────
  const initial = await post("/api/public/elevenlabs/tools/save-quote-snapshot", {
    ...specCtx, quote_stage: "INITIAL", total_amount: 1500, currency: "USD",
    terms: "standard terms, no stair fee",
  }, toolHeaders);
  console.log("initial snapshot:", initial.status, initial.json?.error ?? "ok");

  // ── 2. FINAL quote ──────────────────────────────────────────────────
  const finalQ = await post("/api/public/elevenlabs/tools/save-quote-snapshot", {
    ...specCtx, quote_stage: "FINAL", total_amount: 1350, currency: "USD",
  ...
  const transcript = [
    ...
    { role: "provider", message: "Alright, final is 1350 and stair fee included.", time_in_call_secs: 14 },
  ...
  const w1 = await post("/api/public/elevenlabs/post-call", webhookBody, { "elevenlabs-signature": sig });
  const w2 = await post("/api/public/elevenlabs/post-call", webhookBody, { "elevenlabs-signature": sig });
  const { count: eventCount } = await admin
    .from("call_webhook_events").select("id", { count: "exact", head: true }).eq("call_id", callId);
  console.log("webhook: first=%d second=%d rows=%d", w1.status, w2.status, eventCount);

  // ── 4. Finalize with an unsupported savings claim ───────────────────
  const fin = await post("/api/public/elevenlabs/tools/finalize-call-outcome", {
    ...specCtx,
    outcome: "negotiation_completed",
    price_changed: true,
    initial_amount: 1500, final_amount: 1350, savings_amount: 999, // wrong on purpose
    terms_changed: true,
  ...
  const { data: transcripts } = await admin.from("call_transcripts")
    .select("speaker, sequence_number, started_at_ms, text")
    .eq("call_id", callId).order("sequence_number");
  const { data: evidence } = await admin.from("quote_evidence")
    .select("evidence_type, support_status").eq("negotiation_id", negotiationId);
  const { data: agentEv } = await admin.from("agent_events")
    .select("event_type, metadata").eq("call_id", callId).eq("event_type", "CALL_FINALIZED").maybeSingle();
  ...
  const checks: Array<[string, boolean]> = [
    ...
    ["transcript 5 rows persisted in order", (transcripts?.length ?? 0) === 5
      && transcripts!.every((r, i) => r.sequence_number === i)],
    ["speakers preserved (agent/provider/user)", transcripts?.[0]?.speaker === "agent"
      && transcripts?.[1]?.speaker === "provider" && transcripts?.[4]?.speaker === "user"],
    ...
    ["changed_terms persisted (stair fee included)", agentEv?.metadata?.changed_terms?.[0] === "stair fee included"],
