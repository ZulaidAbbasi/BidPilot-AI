...
async function main() {
  ...
  };
  {
    ...
    record(
      ...
      f1.status === 200 &&
        f2.status === 200 &&
        Number(call?.verified_savings_amount) === 400,
  ...
  // ── 15. RLS isolation between two users ───────────────────────────
  {
    ...
    const bReadA = await bClient
      .from("negotiations")
      .select("id")
      .eq("id", a.negotiationId);
    const bReadACalls = await bClient
      .from("calls")
      .select("id")
      .eq("id", a.callId);
