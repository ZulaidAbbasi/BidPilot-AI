...
export function labelForPath(path: string): string {
  ...
  if (path.startsWith("inventory")) return `Inventory ${humanisePath(path.slice("inventory".length))}`;
...
export function stepForPath(path: string): WizardStep {
  ...
  if (path.startsWith("inventory") || path.startsWith("fragile_items") || path.startsWith("specialty_items"))
    return 3;
...
export function friendlyIssues(issues: readonly ZodIssue[]): FriendlyIssue[] {
