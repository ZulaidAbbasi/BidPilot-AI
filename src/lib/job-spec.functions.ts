...
import { JobSpecSchema } from "./job-spec";
...
export const confirmJobSpec = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // 3. Strict validation. This is where a half-filled draft is rejected.
    const parseResult = JobSpecSchema.safeParse(draftRow.specification);
