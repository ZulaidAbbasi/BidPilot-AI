...
export async function persistCallReconciliation(callId: string) {
    ...
    await Promise.all(
      quoteIds.map((quoteId) => {
        ...
        const verification_status =
          status === "supported" ? "verified" : status === "contradictory" ? "flagged" : "unverified";
  ...
  const needsReview =
    transcripts.length === 0 || result.contradictions > 0 || redFlags.length > 0 || criticalMissingEvidence;
  ...
  if (call.final_outcome) {
    ...
    targetStatus = needsReview ? "needs_review" : successful ? "completed" : "failed";
  }
