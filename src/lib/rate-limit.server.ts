...
async function consume(bucket: string, limit: number, windowSeconds: number): Promise<RateLimit> {
  try {
...
export function rateLimitResponse(retryAfter: number): Response {
  ...
  return new Response(JSON.stringify({ error: "rate_limited", retry_after_seconds: secs }), {
    status: 429,
    headers: {
...
export const RATE_LIMITS = {
  ...
  "load-intake-context": { valid: 30, invalid: 20 },
  "save-intake-patch": { valid: 120, invalid: 20 },
  "finalize-intake-session": { valid: 10, invalid: 20 },
  "intake-post-call": { signed: 60, unsigned: 30 },
} as const;
