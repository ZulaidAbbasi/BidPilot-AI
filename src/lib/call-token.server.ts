...
export async function authorizeCallToolRequest(
  ...
  // Strict lookup: token_hash is UNIQUE. We locate the token row by its hash,
  // then verify it is bound to the exact call and provider claimed in the
  // request payload. This prevents replay of a token issued for a different
  // call, even if the attacker forges the payload's callId to match.
  ...
  if (tokenRow.call_id !== params.callId) {
    return { ok: false, response: jsonResponse(401, { error: "invalid_token" }) };
  }
  // Constant-time compare on the returned hash as a defence-in-depth check.
