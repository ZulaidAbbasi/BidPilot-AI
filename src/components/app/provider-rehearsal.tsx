...
const REHEARSAL_STYLE_META: Record<RehearsalStyle, { label: string; description: string }> = {
  flexible: {
    label: "Flexible",
  ...
  const leverageOptions = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("quotes")
        .select(
          "id, total_amount, currency, quote_stage, captured_at, spec_hash, provider_id, final_confirmed_at, verification_status, providers(name)",
        )
        .eq("negotiation_id", negotiationId)
  ...
  const callStatus = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        transcriptCount: (transcripts as unknown as { count?: number } | null)?.count ?? 0,
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {callMode === "NEGOTIATION" ? (
            <div className="space-y-1">
              ...
              {(leverageOptions.data ?? []).length === 0 && !leverageOptions.isLoading ? (
  ...
  return (
    <div className="rounded-md border p-3">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
        ...
        <Metric
          label="Price changed"
          value={data.verifiedPriceChanged == null ? "—" : data.verifiedPriceChanged ? "yes" : "no"}
        />
        <Metric
          label="Terms changed"
          value={data.verifiedTermsChanged == null ? "—" : data.verifiedTermsChanged ? "yes" : "no"}
        />
