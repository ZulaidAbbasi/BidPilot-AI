...
import { emptyDraft, type JobSpec, type JobSpecDraft } from "@/lib/job-spec";
...
async function loadConfirmedVersions(negotiationId: string): Promise<ConfirmedVersion[]> {
...
export function SpecConfirmationPanel({
  ...
  // Shared validator — Review, Confirm, and the wizard completion bar all
  // agree because they consult exactly this one function.
  const validation = useMemo(() => validateForConfirm(draft), [draft]);
  const strict = useMemo(
    () =>
      validation.ok
        ? { success: true as const, data: validation.spec }
        : {
  ...
  const preview = useQuery({
    ...
    queryFn: () => canonicalizeAndHash((strict as { data: JobSpec }).data),
    staleTime: Infinity,
  });
  ...
  const confirm = useMutation({
    ...
    onSuccess: (result) => {
      setSlowConfirm(false);
      toast.success(`Specification v${result.version} confirmed · ${shortHash(result.hash)}`);
      setDialogOpen(false);
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-3 space-y-0">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Confirmation locks the specification with a SHA-256 hash. Every provider quotes
              against this exact document. Drafts stay editable — confirming again creates a new
              version.
            </p>
          </div>
          {latest && (
            <Badge variant="secondary" className="gap-1.5 whitespace-nowrap">
              <CheckCircle2 className="size-3.5 text-emerald-600" />v{latest.version} ·{" "}
              {shortHash(latest.specification_hash ?? "")}
            </Badge>
          )}
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button type="button" onClick={() => setDialogOpen(true)} disabled={!canConfirm}>
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {v.confirmed_at ? new Date(v.confirmed_at).toLocaleString() : "—"} ·{" "}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Once confirmed, this version cannot be edited or deleted. Providers will quote against
              this exact document.
            </DialogDescription>
          </DialogHeader>
          ...
          {slowConfirm && (
            <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
              Still confirming. This should finish shortly; if it does not, the request will stop
              automatically so you can retry.
            </div>
          )}
          <DialogFooter>
            <Button
            ...
            <Button type="button" onClick={() => confirm.mutate()} disabled={!canConfirm}>
              {confirm.isPending ? (
                <>
...
function ReviewSummary({ spec }: { spec: JobSpec }) {
  const stopsCount = spec.additional_stops.length;
  const stopsSummary =
    stopsCount === 0
      ? "None"
      : `${stopsCount} · ${spec.additional_stops
          .slice(0, 3)
          .map((s) => `${s.label} (${s.purpose})`)
          .join(", ")}${stopsCount > 3 ? "…" : ""}`;
  const packingSummary = `${spec.packing_level}${
    spec.packing_level === "partial" && spec.partial_packing_notes
      ? ` — ${spec.partial_packing_notes.slice(0, 40)}${
          spec.partial_packing_notes.length > 40 ? "…" : ""
        }`
      : ""
  }${spec.unpacking_requested ? " · unpacking" : ""}${
    spec.disassembly_required ? " · disassembly" : ""
  }${spec.reassembly_required ? " · reassembly" : ""}`;
  const rows: [string, string][] = [
    ...
    [
      "Storage",
      spec.storage.needed
        ? `yes · ${spec.storage.duration_days}d${
            spec.storage.climate_controlled ? " · climate" : ""
          }`
        : "no",
    ],
  ];
