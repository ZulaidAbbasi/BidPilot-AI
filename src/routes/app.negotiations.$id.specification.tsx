...
async function loadDraft(negotiationId: string): Promise<{
  ...
  const { data, error } = await supabase
    ...
    .select("id, negotiation_id, specification, completion_percent, updated_at")
...
function SpecEditor({
  ...
  // flight — the pending call carries the newest values.
...
function AdditionalStopsSection({ form }: { form: F }) {
  ...
  return (
    <Section
      ...
      description="Extra pickups, drop-offs, or storage stops between origin and destination. Order matters — movers price by route."
    >
      <ArrayEditor
        title="Stops"
        ...
        onRemove={(i) => stops.remove(i)}
        renderRow={(i) => (
          <div className="grid flex-1 gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,2fr)_minmax(0,1fr)_80px]">
            <FormField
              control={form.control}
            ...
            <FormField
              control={form.control}
              name={`additional_stops.${i}.stop_order`}
              render={({ field }) => (
                <FormItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        placeholder="Gate code, contact, timing constraints…"
                        {...field}
                        value={field.value ?? ""}
                      />
                    </FormControl>
  ...
  return (
    ...
    >
      ...
      <ArrayEditor
        ...
        addLabel="Add item"
        onAdd={() =>
          inv.append({ id: newItemId(), label: "", quantity: 1, notes: "" })
        }
        items={inv.fields}
        onRemove={inv.remove}
        renderRow={(index) => (
                ...
                <FormItem>
                  <FormControl>
                    <Input
                      ...
                      {...field}
                      value={field.value ?? 1}
                      onChange={(e) => field.onChange(e.target.value === "" ? 1 : Math.max(1, Number(e.target.value) || 1))}
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
...
function AccessBlock({ form, prefix }: { form: F; prefix: "origin_access" | "destination_access" }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      <FormField
        control={form.control}
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel>Carry distance (m)</FormLabel>
            <FormControl>
              <Input
                type="number"
                min={0}
      ...
      <CardContent className="space-y-6">
        ...
        <div>
          ...
          <p className="mb-2 text-xs text-muted-foreground">
            Every item below is off by default. Turn one on only if you truly want the agent to have that authority.
          </p>
          <div className="space-y-2">
