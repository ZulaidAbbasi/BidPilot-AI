  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // If the customer clicked "End call" before the webhook landed, the
        // call sits in `ending`. Move it to `processing` so terminal transitions
        // from persistCallReconciliation are legal.
        if (preCall?.status === "ending") {
          await supabaseAdmin
            .from("calls")
        ...
        // Non-terminal provisional status. Terminal transitions are owned by
        // persistCallReconciliation (invoked below when finalize has landed).
        const provisionalStatus = !isSuccess
          ? preCall?.status === "ending" || preCall?.status === "processing"
            ? "processing"
