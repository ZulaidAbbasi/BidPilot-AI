  ...
  .object({
    ...
    valid_until: z
      .preprocess((value) => {
        ...
        const m = s.toLowerCase().match(/(\d+|one|two|three|four|five|six|seven|eight|nine|ten|fourteen|thirty)\s*(day|days|week|weeks|month|months)/);
        if (m) {
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // spoken form wins.
        const total = reconcileSpokenAmount(parsed.total_amount ?? null, parsed.total_words ?? null);
        ...
        const insertRow = {
          ...
          metadata: {
            leverage_quote_id: parsed.leverage_quote_id ?? null,
