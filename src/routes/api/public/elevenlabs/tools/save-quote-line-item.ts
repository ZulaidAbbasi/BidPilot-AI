  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // Reject empty placeholder items: no amount, no provider utterance,
        // no conditional flag, no evidence span, and inclusion at the default.
        // These add rows the reconciler cannot verify and pollute Evidence UI
        // with false "missing evidence" verdicts.
        const evidenceHasSpan = Boolean(
          (parsed.evidence as { transcript_span?: string } | undefined)?.transcript_span?.trim(),
        );
        const isEmptyPlaceholder =
          parsed.amount == null &&
          !parsed.provider_words?.trim() &&
          !parsed.amount_words?.trim() &&
          !parsed.condition_text?.trim() &&
        ...
        if (isEmptyPlaceholder) {
          return jsonResponse(422, {
            ...
            message:
              "Line item must state an amount, an inclusion confirmation with provider words or an evidence span, or a conditional/excluded status. Skipping.",
          });
        }
