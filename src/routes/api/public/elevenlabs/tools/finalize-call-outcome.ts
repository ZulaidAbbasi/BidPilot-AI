...
import {
  authorizeCallToolRequest,
  extractCallToken,
  jsonResponse,
} from "@/lib/call-token.server";
import {
  reconcile,
  type LineItemRow,
  type QuoteRow,
} from "@/lib/call-reconciliation.server";
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        let lineItemsByQuote: Record<string, LineItemRow[]> = {};
        if (quoteIds.length) {
              ...
              const verification_status =
                status === "supported"
                  ...
                  : status === "contradictory"
                    ? "flagged"
                    : "unverified";
              return supabaseAdmin.from("quotes").update({ verification_status }).eq("id", quote.id);
            }),
          );
        }

        // Load current call to compute allowed transition.
        const { data: cur } = await supabaseAdmin
          .from("calls")
          .select("status, webhook_received_at, final_outcome")
          .eq("id", call.id)
          .maybeSingle();
        ...
        // Idempotent-retry guard: if this call was already finalized with a
        // success outcome and the retry claims `negotiation_failed`, keep the
        // earlier success. Prevents a transient tool-orchestration retry from
        // downgrading a real quote capture into a hard failure.
        const effectiveOutcome =
          priorWasSuccess && body.outcome === "negotiation_failed"
            ? (priorOutcome as typeof body.outcome)
            : body.outcome;

        const webhookLanded = cur?.webhook_received_at != null || transcriptRecovered;
        const outcomeIsSuccess =
          effectiveOutcome === "quote_received" || effectiveOutcome === "negotiation_completed";
        const targetStatus: string = needsReview
          ? "needs_review"
          : webhookLanded && outcomeIsSuccess
            ? "completed"
            : outcomeIsSuccess
              ? "quote_captured"
              : "failed";
        ...
        const cleanUnresolved = body.unresolved_questions.map(sanitize).filter(Boolean);
        ...
        let persistedReconciliation: Awaited<ReturnType<typeof persistCallReconciliation>> | null = null;
        try {
