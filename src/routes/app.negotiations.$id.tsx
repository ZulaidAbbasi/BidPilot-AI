...
import { Calendar, MapPin, ShieldCheck, Users, PhoneCall, FileText, ClipboardCheck } from "lucide-react";
                  ...
                  {(n.origin_address || n.destination_address) && (
                    <span className="inline-flex items-center gap-1.5">
                      ...
                      <span className="max-w-[22rem] truncate">
                        {n.origin_address ?? "—"} <span className="mx-1 opacity-50">→</span> {n.destination_address ?? "—"}
                      </span>
                    </span>
                  )}
                  <span className="inline-flex items-center gap-1.5">
            ...
            {data && (
              <div className="hidden shrink-0 items-stretch gap-0 divide-x divide-border rounded-xl border border-border bg-card shadow-[var(--shadow-card)] xl:flex">
                ...
                <MetricChip
                  icon={Users}
                  label="Providers"
                  value={String(data.providerCount)}
                />
                <MetricChip
                  icon={PhoneCall}
                  label="Calls"
...
function MetricChip({
  ...
  return (
    <div className="flex min-w-[112px] flex-col justify-center px-4 py-2.5">
      ...
      <div className={`mt-1 text-[15px] font-semibold tabular-nums ${valueColor}`}>
        {value}
      </div>
    </div>
  );
