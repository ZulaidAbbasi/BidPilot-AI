...
function ReportPage() {
  ...
  const sync = useMutation({
    ...
    onSuccess: (result) => {
      toast.success(`Synced ${result.reconciledCalls} call${result.reconciledCalls === 1 ? "" : "s"}`);
  ...
  const { ranked, winner, verifiedSavings, changedTermLinks, nonWinnerOutcomes, totals, confirmedSpec } = report;
    ...
    <PageBody>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          value={money(verifiedSavings)}
          icon={ShieldCheck}
          tone={verifiedSavings > 0 ? "verified" : "default"}
          hint="Computed server-side from supported quotes"
        />
        <MetricCard
          label="Matched calls"
        ...
        <MetricCard
          label="Ranked offers"
          value={ranked.filter((r) => r.eligibleForWinner).length}
          icon={Sparkles}
      ...
      {!winner ? (
        <Card className="mt-6 border-warn/40 bg-warn-soft/40">
          <CardContent className="flex items-start gap-3 py-4 text-sm">
            ...
            <div>
              <p className="font-medium text-warn-foreground">No winner declared</p>
              <p className="mt-1 text-muted-foreground">
                None of the captured outcomes meets the challenge evidence bar. Reasons per
                candidate appear below. Run additional calls or Sync to recover transcripts.
              </p>
            </div>
          </CardContent>
        </Card>
